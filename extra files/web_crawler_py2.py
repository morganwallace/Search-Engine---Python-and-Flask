...
def Relative_URL_Checker(url,originator_url):
    if url[0] == '/':
        ...
        new_full_url = urljoin(hostname, parsed_url.path)
        return new_full_url
    else: 
    	return url
...
def crawl(count=0):
	...
	if count<40:
		crawl(count)
	saveData(webPageDataDict,'webPageDataDictFile.p') #after crawling save the results
...
def loadData(name):
	webPageDataDictFile=open(name,'rb')
...
def main():
	...
	webPageDataDict=loadData('webPageDataDictFile.p')
	makeInverseIndex()
	saveData(inverseIndex,"InverseIndex.p") #save the Inverted Index for use later
